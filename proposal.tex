...
\begin{document}
    ...
    \end{itemize}  
    Phương pháp đề xuất trong khóa luận nhắm tích hợp các embeddings sau khi được vào huấn luyện ở mô hình \textit{SynGCN}+\textit{SemGCN) sẽ được sử dụng để huấn luyện mô hình  \textit{transformer}  được đề xuất ở (\cite{ \textit{transformer} }) với kì vọng sẽ tăng được hiệu xuất của mô hình.
