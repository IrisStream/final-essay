...
\textit{SynGCN} và \textit{SemGCN) sử dụng thuật toán Adam với giá trị của learning rate:
\begin{equation*}
	l_{rate} = 10^{-3}
\end{equation*}

Đối với  \textit{transformer} , thuật toán tối ưu được sử dụng vẫn là  \textit{transformer}  với các siêu tham số như sau:
\begin{equation*}
	\begin{cases}
		l_{rate} = 10^{-3} \\
		\beta_1 = 0.9 \\
...
\begin{itemize}
	...
	\item Kích thước embedding được huấn luyện ở mô hình WordGCN chưa phù hợp khi đưa vào mô hình  \textit{transformer} .
\end{itemize}